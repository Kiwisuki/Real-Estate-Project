...
def predict(row, fold):
    ...
    y  = df['Price']
    with open(f"pickles/rent_model.pickle", "rb") as f:
        model = pickle.load(f)

    x = X.values[0]
    y = y.values[0]
    rent_pred = int(round(np.expm1(model.predict(X)[0]), -1))



    X = df.drop(['Price' ,'eur_m2'], axis=1)
    y  = df['Price']
    with open(f"pickles/model_{fold}.pickle", "rb") as f:
        model = pickle.load(f)

    x = X.values[0]
    y = y.values[0]
    price_pred = int(round(np.expm1(model.predict(x)), -2))
    price = int(y)

    return rent_pred, price_pred, price
...
def get_fold(id_):
    with open("pickles/fold_ids.pickle", "rb") as f:
        ids = pickle.load(f)
